...
- `src/` contains all Rust source files.
  ...
  - `src/config.rs` loads environment variables into per-area sub-structs (`CONFIG.gemini.api_key`, `CONFIG.openrouter.base_url`, `CONFIG.db.url`, `CONFIG.agents.tldr_chunk_size`, …; one `load_<group>()` per group); unparsable values warn and fall back, every provider endpoint is validated, and `Config::secret_values()` lists every credential for log redaction. `src/prompts.rs` holds the system-prompt text.
  ...
  - `src/handlers/` holds one file per command family (`image.rs` with `image/selection.rs` and shared `image_delivery.rs`, `tldr.rs`, `factcheck.rs`, `persona.rs`, `mysong.rs`, `token_stats.rs`, `admin.rs`, `help.rs`, `codex_admin.rs`), the `/q` family as a module directory (`qa/{trigger,model_resolution,selection_ui,prompt,chat_search,process,handler,tests}.rs`), plus access control (`access.rs`), link/media enrichment (`content.rs`, `media.rs`), and response delivery (`responses.rs`).
  - `src/llm/` integrates Gemini, OpenRouter/NVIDIA/Ollama, OpenAI Responses (`responses_provider/{payload,sse,transport,codex_identity,tool_loop}.rs`), and ChatGPT Codex providers; `transport/` is the shared retry/body/SSE layer, `tool_loop.rs`/`tool_runtime.rs` the tool loop, `resolved_model.rs` execution snapshots and pinned model metadata, `text_model.rs` configured/resolved text-model calls, `prompting.rs` chat-history fencing, `runtime_models.rs` the model catalog with `codex_selected_model.rs` holding the persisted Codex selection, plus web search helpers.
...
- **Persistence (`db/`).** `schema::migrate` stamps `PRAGMA user_version` and applies numbered migrations at startup (v1 is the idempotent `CREATE IF NOT EXISTS` schema, so existing databases are stamped, not rebuilt). Incoming messages are recorded through an async write queue (batched by `DB_WRITE_BATCH_SIZE`/`DB_WRITE_FLUSH_MS`) rather than written inline. `db/search.rs` maintains the versioned chat-search normalization (jieba tokenization) backing `/s` and `/qc`; `search_index.rs` performs resumable cursor backfill with transient-lock retries. Normalization v3 retains 4,000-character semantic and 8,000-character token budgets and adds canonical chat tags to every MATCH. SQLite schema v2 uses external-content FTS with version-gated old-value delete triggers; migration and version stamps are transactional. Backfill checks external-content integrity without a second rebuild. Older binaries require restoring a consistent pre-upgrade backup.
...
- **Execution contracts.** Picker callbacks revalidate before freezing a resolved model. Use `TextCallRequest` and `StepCallRequest` for narrow named call inputs. Pass that resolved configuration and Codex metadata through agent phases and provider calls; do not look it up by ID again. Share one `WallClock` across text generation and legacy fallback, and run post-status work through `run_with_status_message`. Keep `ResponseContent` as original Markdown plus literal metadata until delivery. External-media body reads and budget admission follow input order.

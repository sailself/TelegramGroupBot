...
const USER_AGENT: &str = "TelegramGroupHelperBot/0.1 (+https://github.com/sailself/TelegramGroupHelperBot)";
...
fn normalize_media_url(url: &str) -> String {
    let parsed = Url::parse(url).or_else(|_| Url::parse(&format!("https:{}", url))).unwrap_or_else(|_| Url::parse("https://x.com").unwrap());
    let mut query_pairs = parsed.query_pairs().collect::<Vec<_>>();
    if parsed.domain().map(|d| d.ends_with("twimg.com")).unwrap_or(false) {
    ...
    if !query_pairs.is_empty() {
        normalized.set_query(Some(&serde_urlencoded::to_string(&query_pairs).unwrap_or_default()));
    }
...
fn collect_relevant_lines(markdown_block: &str) -> Vec<String> {
    ...
    for line in markdown_block.lines() {
        ...
        if stripped.is_empty() {
            if relevant.last().map(|line| !line.is_empty()).unwrap_or(false) {
        ...
        let stop_prefixes = ["watch on", "show more", "related", "more replies", "explore", "tweet your reply"];

        if stop_markers.iter().any(|marker| marker == &lowered)
            || stop_prefixes.iter().any(|prefix| lowered.starts_with(prefix))
        {
            break;
        }
...
fn clean_lines_and_media(lines: &[String]) -> (Vec<String>, Vec<String>, Vec<String>) {
    ...
    let profile_media_tokens = ["profile_images", "profile_banners", "semantic_core_img", "/emoji/"];
    ...
    for line in lines {
        ...
        for caps in media_pattern.captures_iter(&working) {
            ...
            if profile_media_tokens.iter().any(|token| media_url.contains(token)) {
                continue;
            }
            if video_extensions.iter().any(|ext| media_url.ends_with(ext)) || media_url.contains("video.twimg.com") {
        ...
        working = link_pattern
            .replace_all(&working, |caps: &regex::Captures| caps[1].trim().to_string())
            .to_string();
        working = Regex::new(r"\s+").unwrap().replace_all(&working, " ").trim().to_string();
        ...
        if let Some(last) = cleaned.last_mut() {
            if !last.ends_with(['.', '!', '?', ':']) && (working.starts_with('@') || working.starts_with('#')) {
                *last = format!("{} {}", last, working);
                continue;
            }
            if punct_prefixes.iter().any(|prefix| working.starts_with(prefix)) {
                *last = format!("{}{}", last, working);
                continue;
            }
        }
...
fn extract_metadata(cleaned_lines: &[String]) -> (Option<String>, Option<String>, Option<usize>) {
    ...
    for (idx, line) in cleaned_lines.iter().take(6).enumerate() {
        if line.starts_with('@') && !line.contains(' ') {
            ...
            if let Some(prev) = cleaned_lines[..idx].iter().rev().find(|line| !line.is_empty()) {
...
fn strip_indices(lines: &[String], indexes: &[Option<usize>]) -> Vec<String> {
    ...
    lines
        .iter()
        .enumerate()
        .filter_map(|(idx, line)| if skip.contains(&idx) { None } else { Some(line.clone()) })
        .collect()
}
...
pub async fn extract_twitter_content(url: &str) -> Result<TwitterContent> {
    ...
    if !response.status().is_success() {
        return Err(anyhow!("Twitter proxy request failed with status {}", response.status()));
    }

    let raw_text = response.text().await?;
    let marker = "Markdown Content:\n";
    let marker_idx = raw_text.find(marker).ok_or_else(|| anyhow!("Unable to locate Twitter/X markdown content in response"))?;
    ...
    let display_index = display_name.as_ref().and_then(|name| cleaned_lines.iter().position(|line| line == name));
    ...
    if !image_urls.is_empty() {
        formatted_content.push_str(&format!("\n\nImages attached: {} image(s)", image_urls.len()));
    }

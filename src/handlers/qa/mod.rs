...
#[cfg(test)]
use crate::state::{PendingQRequest, QaCommandMode};
...
use prompt::{
    ...
    build_system_prompt, extract_youtube_urls_for_available_models, prepare_youtube_inputs_for_qa,
};
...
#[cfg(test)]
use selection_ui::{
    create_model_selection_keyboard_with_models, resolve_model_callback_token_with_models,
    take_pending_q_request_for_callback, PendingQRequestCallbackAction,
...
use model_resolution::{
    add_explicit_quick_model, codex_quick_result_label,
    resolve_prepared_quick_text_model_with_models, resolve_quick_text_model_with_models,
    result_model_display_name, selectable_model_ids_for_request_with_models,
...
#[cfg(test)]
use process::{
    append_quick_search_footer, qa_mode_label, reasoning_override_for_qa_mode,
    uses_quick_tool_runtime, QUICK_SEARCH_FOOTER,
};

mod handler;
#[cfg(test)]
use handler::build_q_command_insert;

//! Unit tests for the Responses provider split (payload, sse, transport,
//! codex_identity, tool_loop).
...
use transport::{
    add_codex_responses_lite_header, call_provider_api_with_auth_hooks, capture_response_metadata,
    merge_request_headers_for_attempt, responses_base_url, responses_request_timeout_secs,
    summarize_error_body, ResponsesRequestDetails, CODEX_RESPONSES_LITE_HEADER,
    CODEX_TURN_STATE_HEADER, MODELS_ETAG_HEADER, REQUEST_ID_HEADER, RESPONSES_MAX_BODY_BYTES,
};
...
fn test_identity(
    ...
    CodexRequestIdentity::resolve_with(config, None, record.as_ref(), "acct-1", reasoning_override)
...
fn explicit_codex_metadata_controls_responses_lite() {
    ...
    let mut explicit_terra_config = model_config(ThirdPartyProvider::OpenAICodex, "gpt-5.6-terra");
...
fn request_identity_rejects_fresh_account_mismatch_from_validated_metadata() {
    ...
    assert!(
        CodexRequestIdentity::resolve_with(&config, None, Some(&record), "acct-2", None).is_err()
    );
    assert!(
...
{
    let mut synthesized_agent_step = model_config(ThirdPartyProvider::OpenAICodex, "gpt-5.4-mini");
    synthesized_agent_step.tools = false;
    let configured_q_model = model_config(ThirdPartyProvider::OpenAICodex, "configured-q-model");

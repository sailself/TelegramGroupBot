pub(crate) use crate::tools::twitter_extractor::test_support::identity;
...
impl ExpectedRequest {
    ...
    pub(crate) fn with_body_fragment(mut self, fragment: &[u8]) -> Self {
        self.body_fragments.push(fragment.to_vec());
        self
    }
...
impl ExpectedRequest {
    fn verify(&self, request: &Request) -> Result<(), String> {
        ...
        for fragment in &self.body_fragments {
            ...
            {
                return Err("missing expected request body fragment".to_string());
            }
        }
...
fn read_request(stream: &mut TcpStream) -> Result<Request, RequestError> {
    ...
    if length > 16 * 1024 * 1024 {
        return Err(RequestError::Assertion(
            "request body exceeds test limit".to_string(),
        ));
    }
    while bytes.len() < header_end + length {
        ...
        if read == 0 {
            return Err(RequestError::ClientDisconnect(
                "request body ended early".to_string(),
            ));
        }
        bytes.extend_from_slice(&chunk[..read]);
    }
